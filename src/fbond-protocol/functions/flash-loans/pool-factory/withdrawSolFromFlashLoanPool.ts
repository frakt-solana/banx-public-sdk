...
type WithdrawSolFromFlashLoanPool = (params: {
    programId: web3.PublicKey;
    connection: web3.Connection;

    args: {
        solAmountToWithdraw: number;
    };
...
export const withdrawSolFromFlashLoanPool: WithdrawSolFromFlashLoanPool = async ({ programId, connection, args, accounts, sendTxn }) => {
    ...
    instructions.push(
        await program.methods
            .withdrawSolFromFlashLoanPool(new BN(args.solAmountToWithdraw))
            .accounts({
                pool: accounts.pool,
