...
export interface borrowStakedBanxPerpetualParamsAndAccounts {
...
type BorrowStakedBanxPerpetual = (params: {
  programId: web3.PublicKey;
  connection: web3.Connection;

  args: {
    perpetualBorrowParamsAndAccounts: borrowStakedBanxPerpetualParamsAndAccounts[];
  };
...
export const borrowStakedBanxPerpetual: BorrowStakedBanxPerpetual = async ({
  programId,
  connection,
  args,
  addComputeUnits,
    ...
    await Promise.all(
      perpetualBorrowParamsAndAccountsWithSeeds.map(async (perpetualBorrowParamsAndAccount) => {
        ...
        const subRemainingAccounts = [
          {
            pubkey: perpetualBorrowParamsAndAccount.banxStake,
            isSigner: false,
            isWritable: true,
          },
          {
  ...
  instructions.push(
    await program.methods
      .borrowStakedBanxPerpetual(perpetualBorrowParams)
  ...
  const optimisticResults: BondAndTransactionAndOfferOptimistic[] = await Promise.all(
    perpetualBorrowParamsAndAccountsWithSeeds.map(async (borrowParam) => {
      const [fbond] = await web3.PublicKey.findProgramAddress(
        [
          ENCODER.encode(FRAKT_BOND_PREFIX),
