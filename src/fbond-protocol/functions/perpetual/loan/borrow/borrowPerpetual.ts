...
import { BondAndTransactionAndOfferOptimistic, nowInSeconds, optimisticBorrowPerpetual } from '../../offer';
import {
  findTokenRecordPda,
...
export const borrowPerpetual: BorrowPerpetual = async ({
  programId,
  connection,
  args,
  addComputeUnits,
    ...
    await Promise.all(
      perpetualBorrowParamsAndAccountsWithSeeds.map(async (perpetualBorrowParamsAndAccount) => {
        ...
        const userTokenAccount = await findAssociatedTokenAddress(
          accounts.userPubkey,
          perpetualBorrowParamsAndAccount.tokenMint,
        );
        const editionInfo = getMetaplexEditionPda(perpetualBorrowParamsAndAccount.tokenMint);
        const nftMetadata = getMetaplexMetadata(perpetualBorrowParamsAndAccount.tokenMint);
        const ownerTokenRecord = findTokenRecordPda(perpetualBorrowParamsAndAccount.tokenMint, userTokenAccount);

        const targetRuleSet = perpetualBorrowParamsAndAccount.ruleSet || accounts.protocolFeeReceiver;
        const targetDelegatePubkey = accounts.protocolFeeReceiver;
        ...
        const subRemainingAccounts = [
          ...
          {
            pubkey: targetRuleSet,
            isSigner: false,
            isWritable: true,
          },
          {
            pubkey: targetDelegatePubkey,
            isSigner: false,
            isWritable: true,
          },
          {
  ...
  instructions.push(
    await program.methods
      .borrowPerpetual(perpetualBorrowParams)
