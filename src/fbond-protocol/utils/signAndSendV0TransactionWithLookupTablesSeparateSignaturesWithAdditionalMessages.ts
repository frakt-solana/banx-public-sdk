...
type SignAndSendV0TransactionWithLookupTablesSeparateSignaturesWithAdditionalMessages = (props: {
  ...
  additionalMessages: web3.VersionedTransaction[];
...
export const signAndSendV0TransactionWithLookupTablesSeparateSignaturesWithAdditionalMessages: SignAndSendV0TransactionWithLookupTablesSeparateSignaturesWithAdditionalMessages =
  async ({
    notBondTxns,
    ...
    try {
      if (isLedger) {
        for (const txnAndSigners of chunk(notBondTxns, TXNS_IN_ONE_SIGN_FOR_LEDGER)) {
          await signAndSendV0TransactionWithLookupTablesSeparateSignaturesWithAdditionalMessages({
      ...
      const { blockhash: blockhashFirst, lastValidBlockHeight } = await connection.getLatestBlockhash();
      ...
      if (fastTrackV0Transactions.length > 0) {
        additionalMessages.map((additionalTx) => {
          fastTrackV0Transactions.push(additionalTx);
        });
      }
      ...
      const transactionsFlatArrLookupTables = [
        ...
        ...txnsAndSignersCreateAndExtendLookupTables.flat().map(({ transaction, signers }) => {
          transaction.recentBlockhash = blockhashFirst;
      ...
      const { blockhash: blockhashSecond } = await connection.getLatestBlockhash();
      ...
      const v0Transactions = await Promise.all(
        v0InstructionsAndSigners.map(async (ixAndSigner) => {
            ...
            (await Promise.all(
              ixAndSigner.lookupTablePublicKeys.map(
                async (lookupTablePublicKey) =>
                  (
                    await connection.getAddressLookupTable(new web3.PublicKey(lookupTablePublicKey.tablePubkey))
                  ).value,
              ),
            )) as web3.AddressLookupTableAccount[];
          // .map(tableAccount => ({
          //   ...tableAccount, state: {
          //     ...tableAccount.state,
          //     lastExtendedSlot: lastSlot,
          ...
          const transactionsMessageV0 = new web3.VersionedTransaction(
            new web3.TransactionMessage({
              payerKey: wallet.publicKey,
              recentBlockhash: blockhashSecond,
              instructions: ixAndSigner.instructions,
      ...
      if (v0Transactions.length > 0) {
        additionalMessages.map((additionalTx) => {
          v0Transactions.push(additionalTx);
        });
      }
      const deactivateLookupTableTxns = v0InstructionsAndSigners
        ...
        .map((transaction) => {
          transaction.recentBlockhash = blockhashSecond;
          transaction.feePayer = wallet.publicKey;
          return transaction;
        });
      ...
      //   );
      console.log('Check if goes before onAfterSend()');
      await new Promise((r) => setTimeout(r, 7000));
      onAfterSend?.();
