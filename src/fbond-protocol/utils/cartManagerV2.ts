...
export const patchBondOfferWithStandartInterest = (bondOffer: BondOfferV2, standartInterest: number): BondOfferV2 => {
  const correctedStandartInterest = getStandartInterest(standartInterest);

  const recalculatedBondsInOrder = bondOffer.fundsSolOrTokenBalance / correctedStandartInterest;

  return {
    ...bondOffer,
    edgeSettlement: recalculatedBondsInOrder,
    baseSpotPrice: correctedStandartInterest,
    currentSpotPrice: correctedStandartInterest,
    bidSettlement: bondOffer.bidCap * -1 + recalculatedBondsInOrder,
  };
};

export const DEFAULT_STANDART_INTEREST = 9800;

export const getStandartInterest = (standartInterest: number) => {
  return standartInterest > 5 ? standartInterest : DEFAULT_STANDART_INTEREST;
};
